...
const sanitizeTaskDates = (rawTasks: any[]): Task[] => {
  ...
  return (
    <div className="flex h-[100dvh] bg-background text-on-background overflow-hidden font-body selection:bg-primary-container selection:text-on-primary-container">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-6 pr-1">
                      ...
                      <div>
                        ...
                        <PeriodicTable
                          ...
                          onViewDetail={(rootId, sample) => setSelectedPeriodicGroup({ rootId, sampleTask: sample })}
                        />
                      </div>
                      ...
                      <div className="pt-2">
                        ...
                        <PeriodicTable
                          tasks={tasks.filter(t => t.recurrence === "QUARTERLY")}
                          allTasks={tasks}
                          onEdit={openEditTaskModal}
                          onViewDetail={(rootId, sample) => setSelectedPeriodicGroup({ rootId, sampleTask: sample })}
                        />
                      ...
                      <div className="pt-2">
                        ...
                        <PeriodicTable
                          tasks={tasks.filter(t => t.recurrence === "SEMI_ANNUALLY")}
                          allTasks={tasks}
                          onEdit={openEditTaskModal}
                          onViewDetail={(rootId, sample) => setSelectedPeriodicGroup({ rootId, sampleTask: sample })}
                        />
                      ...
                      <div className="pt-2">
                        ...
                        <PeriodicTable
                          tasks={tasks.filter(t => t.recurrence === "ANNUALLY")}
                          allTasks={tasks}
                          onEdit={openEditTaskModal}
                          onViewDetail={(rootId, sample) => setSelectedPeriodicGroup({ rootId, sampleTask: sample })}
                        />
      ...
      {/* Periodic Group Detail Modal */}
      <AnimatePresence>
        {selectedPeriodicGroup && (
          <PeriodicGroupDetailModal
...
function PeriodicTable({
  ...
  allTasks: Task[];
  onEdit: (t: Task) => void;
  onViewDetail: (rootId: string, sampleTask: Task) => void;
}) {
  if (tasks.length === 0) {
    return (
...
function PeriodicGroupDetailModal({
  groupInfo,
  tasks,
  onEditTask,
  onDeleteTask,
  ...
  const formatRangeSummary = () => {
    if (groupTasks.length === 0) return "설정된 일정 없음";
    const firstDate = parseISO(groupTasks[0].deadline);
    const lastDate = parseISO(groupTasks[groupTasks.length - 1].deadline);

    switch (sampleTask.recurrence) {
      case "WEEKLY":
        return `반복 기간: ${format(firstDate, "yyyy년 MM월 dd일", { locale: ko })} ~ ${format(lastDate, "yyyy년 MM월 dd일", { locale: ko })}`;
      case "MONTHLY":
      case "QUARTERLY":
      case "SEMI_ANNUALLY":
        return `반복 기간: ${format(firstDate, "yyyy년 MM월", { locale: ko })} ~ ${format(lastDate, "yyyy년 MM월", { locale: ko })}`;
      case "ANNUALLY":
        return `반복 기간: ${format(firstDate, "yyyy년", { locale: ko })} ~ ${format(lastDate, "yyyy년", { locale: ko })}`;
      default:
        return `반복 기간: ${format(firstDate, "yyyy년 MM월 dd일", { locale: ko })} ~ ${format(lastDate, "yyyy년 MM월 dd일", { locale: ko })}`;
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        animate={{ scale: 1, opacity: 1 }}
        exit={{ scale: 0.95, opacity: 0 }}
        className="relative bg-surface w-full max-w-2xl rounded-2xl shadow-xl overflow-hidden max-h-[85vh] flex flex-col border border-border z-10"
      >
        {/* Header */}
        <div className="flex justify-between items-center p-5 border-b border-border bg-surface-variant/30 flex-shrink-0">
          <div>
            <div className="flex items-center gap-2">
        ...
        {/* Task List */}
        <div className="p-5 flex-1 overflow-y-auto space-y-3 cell-scroll">
          {groupTasks.length === 0 ? (
        ...
        {/* Footer */}
        <div className="p-4 border-t border-border bg-surface-variant/20 flex justify-between items-center flex-shrink-0">
          <button
            onClick={() => {
          ...
          <button
            onClick={onClose}
            className="px-5 py-2 text-xs font-bold rounded-xl bg-primary text-on-primary hover:opacity-90 transition-colors shadow-sm"
          >
            닫기
          </button>
        </div>
